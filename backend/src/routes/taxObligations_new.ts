...
router.get('/', authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.info('Buscando obrigações tributárias para o usuário', { userId });
    const [rows] = await db.query<RowDataPacket[]>('SELECT * FROM tax_obligations WHERE user_id = ? ORDER BY due_date ASC', [userId]);
...
router.post('/', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    console.error('❌ Erro ao criar obrigação tributária:', error);
...
router.put('/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const { id } = req.params;
    const { title, description, amount, dueDate, taxType, status, referenceMonth, notes, complianceDate, category } = req.body;
    const [result] = await db.query(
      `UPDATE tax_obligations SET name=?, description=?, amount=?, due_date=?, type=?, tax_type=?, status=?, reference_month=?, notes=?, compliance_date=? WHERE id=? AND user_id=?`,
      [title, description, amount, dueDate, category, taxType, status, referenceMonth, notes, complianceDate, id, userId]
    );
